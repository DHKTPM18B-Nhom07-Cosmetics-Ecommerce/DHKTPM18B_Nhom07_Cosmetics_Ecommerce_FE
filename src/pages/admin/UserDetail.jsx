    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // === GỌI API CHECK RỦI RO NGAY KHI LOAD TRANG ===
                try {
        ...
        <div className="admin-user-detail-page p-8 bg-[#F8F9FA] min-h-screen font-sans">
            ...
            <div className="bg-[#D5E2E6] rounded-2xl p-6 mb-6 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center">
                <div>
                    ...
                    <p className="text-gray-500 mt-1">
                        {isCustomer ? 'Mã KH:' : 'Mã NV:'} <span className="font-medium text-gray-700">#{user.id}</span>
                    </p>
          ...
          <span className={`px-4 py-1.5 rounded-full text-sm font-semibold ${
              user.status === 'ACTIVE' ? 'bg-green-100 text-green-700' :
                  user.status === 'LOCKED' || user.status === 'DISABLED' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700'
          }`}>
            {user.status === 'ACTIVE' ? 'Hoạt động' : (user.status === 'DISABLED' ? 'Vô hiệu hóa' : user.status)}
          </span>
                </div>
            </div>
                    ...
                    <div className="bg-[#D5E2E6] rounded-2xl p-6 shadow-sm relative">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-8">
                            ...
                            <div>
                                ...
                                <div className="flex items-center gap-2 font-semibold text-gray-800">
                                    <Mail className="w-4 h-4 text-gray-500" />{user.username || '—'}
                                </div>
                            </div>
                    ...
                    <div className="bg-[#D5E2E6] rounded-2xl p-6 shadow-sm">
                        ...
                        {!isCustomer && (
                            <button
                                ...
                                className="flex mt-4 w-full py-3 items-center gap-1 px-3 py-1 bg-[#2B6377] text-white rounded-lg font-medium hover:bg-opacity-90 transition flex items-center justify-center gap-2"
                            >
                                <Edit className="w-3.5 h-3.5" /> Chuyển đổi quyền
                            </button>
                        )}
                    </div>
                </div>
